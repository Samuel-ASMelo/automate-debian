...
for user_info in "${USERS[@]}"; do
    ...
    if ! id "$user_name" &>/dev/null; then
        if [ -n "$extra_group" ]; then
            useradd -m -g "$primary_group" -G "$extra_group" "$user_name"
        else
...
if ! getfacl "$html_file" | grep -q "group:operacoes:r-x"; then
    setfacl -m g:operacoes:r-x "$html_file"
...
if ! grep -q "DocumentRoot /srv/app" "$apache_conf"; then
    sed -i 's|DocumentRoot /var/www/html|DocumentRoot /srv/app|g' "$apache_conf"
...
if ! grep -q "<Directory /srv/app/>" "$apache_conf"; then
    sed -i '/<Directory \/var\/www\/>/i <Directory \/srv\/app\/>\n    Options Indexes FollowSymLinks\n    AllowOverride None\n    Require all granted\n</Directory>' "$apache_conf"
